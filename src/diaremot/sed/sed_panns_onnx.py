...
def _write_events_csv(output_path: Path, file_id: str, events: Sequence[dict[str, float | str]]) -> None:
    ...
    with output_path.open("w", encoding="utf-8") as handle:
        for frame_idx, (time_sec, topk, probs) in enumerate(
            ...
            payload = {
                ...
                "coarse_topk": topk,
                "coarse_posteriors": {
                    label: float(prob) for label, prob in zip(debug.coarse_labels, probs, strict=False)
                },
            }
...
def _build_arg_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--frame", type=float, default=1.0, help="Frame length in seconds (default: 1.0)")
    parser.add_argument("--hop", type=float, default=0.5, help="Frame hop in seconds (default: 0.5)")
    parser.add_argument(
    ...
    parser.add_argument(
        "--min-dur", dest="min_dur", type=float, default=0.30, help="Minimum event duration in seconds"
    )
    parser.add_argument(
        "--merge-gap",
        dest="merge_gap",
    ...
    parser.add_argument("--mel", dest="mel", type=int, default=64, help="Number of mel bins (informational)")
    parser.add_argument("--topk", dest="topk", type=int, default=3, help="Top-K labels cached per frame")
    parser.add_argument("--threads", dest="threads", type=int, help="Override ONNX Runtime thread count")
    parser.add_argument(
        "--debug-jsonl",
        dest="debug_jsonl",
