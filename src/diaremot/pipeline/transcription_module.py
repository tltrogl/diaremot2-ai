...
class AsyncTranscriber:
    ...
    def _faster_whisper_transcribe(
        ...
        def _call_transcribe(use_vad: bool):
            kw = dict(
                language=language,
        ...
        try:
            segments, info = _call_transcribe(prefer_vad)
        except Exception as e:
            msg = str(e).lower()
        ...
        if not text_parts and prefer_vad:
            try:
                # Retry without VAD; some short clips become empty with VAD
                segments, info = _call_transcribe(False)
                text_parts, words, logprobs = _collect(segments)
            except Exception as e:
                self.logger.error(f"faster-whisper retry without VAD failed: {e}")
                return None
